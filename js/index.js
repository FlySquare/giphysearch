...
function ajaxValidate(ajax){
  var valid = 0;
  
  if(ajax.method.toLowerCase() === 'get' || ajax.method.toLowerCase() === 'post'){
...
function ajaxQuery(ajax){
  if($('.wrapper').hasClass('ui-1')){
    $('.wrapper').removeClass('ui-1');
    $('.wrapper').addClass('ui-2');
  }
  var validAjax = ajaxValidate(ajax);
...
function populateResults(data){
  ...
  for(var i in data){
    ...
    $('.results').append('<a class="still"><img src="'+img.url+'" alt="'+ alt +'" data-state="still" data-still="'+ img.url +'" data-gif="'+ gif.url +'"></a>');
    
    if(i >= data.length - 1){
      $(".results").justifiedGallery({
        rowHeight : 150,
        lastRow : 'nojustify',
        margins : 13,
        randomize: true,
...
$('.query').keyup(function(e){
  ...
  timeout = setTimeout(function(){
    $('.title').html('Results for: <span>'+$('.query').val()+'</span>');
    ajaxQuery(ajax);
  }, 500);
});
...
$('select').each(function(){
  ...
  that.after('<ul class="select-list"></ul>');
  ...
  that.children('option').each(function(){
    ...
    if($(this).is(':selected')){
      that.after('<div class="select-active" data-val="'+val+'"><p>'+text+'</p></div>');
    }

    that.siblings('.select-list').append('<li>'+text+'</li>');
  });
...
function selectClick(){
  $('.select-active').click(function(e){
    ...
    $(this).siblings('.select-list').slideToggle('500');
  });
  ...
  $('.select-list li').click(function(e){
    ...
    $(this).parent('.select-list').slideToggle('500');
    $(this).parent('.select-list').siblings('.select-active').toggleClass('dropped');
    $(this).parent('.select-list').siblings('select').children('option').attr('selected',false);
    $(this).parent('.select-list').siblings('select').children('option[value="'+val+'"]').attr('selected',true);
    $(this).parent('.select-list').siblings('.select-active').children('p').text(text);
  });
}
